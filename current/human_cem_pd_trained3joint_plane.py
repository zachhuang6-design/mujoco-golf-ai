...
import golf_3joint_plane_common
...
from golf_3joint_plane_common import (
    ...
    EPISODE_STEPS,
    IMPACT_ELBOW_TARGET,
    IMPACT_ELBOW_TOLERANCE,
    TOP_SET_POSE,
    TOP_SET_TOLERANCE,
    get_plane_launch_profile,
    make_plane_model,
    print_plane_setup,
)
...
PARAMS = [
    ("top_step", 220.0, 440.0),
    ("top_hold", 70.0, 210.0),
    ("impact_gap", 150.0, 380.0),
    ("finish_gap", 60.0, 270.0),
    ("elbow_lag", 15.0, 135.0),
    ("wrist_lag", 35.0, 180.0),
    ("impact_s", -0.55, 0.35),
    ("impact_e", math.radians(6.0), math.radians(28.0)),
    ("impact_w", -0.65, 0.25),
    ("finish_s", -1.40, 0.60),
    ("finish_e", math.radians(0.0), math.radians(80.0)),
    ("finish_w", -1.30, 0.90),
]
...
def candidate_to_vector(candidate):
    ...
    return [
        ...
        float(candidate["impact_step"] - down_start_step),
...
def target_angles(candidate, step):
    address = candidate.get("address_pose", ADDRESS_POSE)
    top = candidate.get("top_pose", TOP_SET_POSE)
...
def address_score(data):
    score = 0.0
    for i in range(3):
        score += angle_score(data.qpos[i], ADDRESS_POSE[i], ADDRESS_TOLERANCE)
    return score / 3.0
...
def top_tracking_score(data, candidate):
    ...
    score = 0.0
    for i in range(3):
        score += angle_score(data.qpos[i], top[i], TOP_SET_TOLERANCE)
    return score / 3.0
...
def top_pose_score(data):
    ...
    score = 0.0
    score += angle_score(shoulder, TOP_SET_POSE[0], TOP_SET_TOLERANCE)
    score += angle_score(elbow, TOP_SET_POSE[1], TOP_SET_TOLERANCE)
    score += angle_score(wrist, TOP_SET_POSE[2], TOP_SET_TOLERANCE)
    score += angle_score(forearm_angle, math.radians(180.0), math.radians(35.0))
    score += angle_score(club_angle, math.radians(270.0), math.radians(40.0))
    return score / 5.0


def impact_line_score(data):
    shoulder = data.qpos[0]
    elbow = data.qpos[1]
    wrist = data.qpos[2]
    forearm_angle = shoulder + elbow
    club_angle = shoulder + elbow + wrist
    score = 0.0
    score += angle_score(club_angle - forearm_angle, 0.0, math.radians(32.0))
    score += angle_score(wrist, 0.0, math.radians(35.0))
    return score / 2.0


def elbow_bend_score(elbow_angle):
    return angle_score(elbow_angle, IMPACT_ELBOW_TARGET, IMPACT_ELBOW_TOLERANCE)


def print_startup_diagnostics(model, club_name, plane_angle_deg):
    ...
    print("Loaded plane shared file:", golf_3joint_plane_common.__file__)
    print_plane_setup(club_name, plane_angle_deg)
...
def simulate_pd_swing(model, candidate, require_hit=True, launch_profile=None):
    ...
    address_sample = address_score(data)
    top_score_sample = 0.0
    top_tracking_sample = 0.0
    impact_line_sample = 0.0
    impact_elbow_score = 0.0
    ...
    for step in range(EPISODE_STEPS):
        ...
        active_downswing = down_start_step <= step <= candidate["impact_step"] + 45
        ...
        if contact_includes(data, club_head_geom_id, ball_geom_id):
            ...
            if first_hit_step is None:
                ...
                impact_elbow_bend = data.qpos[1]
                impact_was_active_downswing = active_downswing
                impact_club_was_forward = impact_club_vx >= MIN_FORWARD_CLUB_SPEED
                impact_line_sample = impact_line_score(data)
                impact_elbow_score = elbow_bend_score(impact_elbow_bend)
    ...
    if candidate["elbow_lag"] >= 0 and candidate["wrist_lag"] > candidate["elbow_lag"]:
        sequence_score += 1.0
    sequence_score += address_sample
    sequence_score += top_score_sample
    sequence_score += top_tracking_sample
    sequence_score += impact_line_sample
    sequence_score += impact_elbow_score
    ...
    reward = (
        ...
        + impact_line_sample * 75.0
        + impact_elbow_score * 180.0
        + sequence_score * 50.0
    ...
    return {
        ...
        "impact_elbow_bend": impact_elbow_bend,
        ...
        "impact_line_score": impact_line_sample,
        "impact_elbow_score": impact_elbow_score,
...
def print_result(prefix, result):
    print(
        ...
        "elbow_deg",
        round(math.degrees(result["impact_elbow_bend"]), 2),
        "elbow_score",
...
def print_pd_candidate(candidate):
    print("\n# Paste this dictionary into human_golf_arm3joint_pd_plane.py as PD_SWING_CANDIDATE:\n")
    ...
    print(f'    "address_pose": ({candidate["address_pose"][0]:.4f}, {candidate["address_pose"][1]:.4f}, {candidate["address_pose"][2]:.4f}),')
...
def train(
    ...
    smoothing=0.7,
    club_name="7iron",
    plane_angle_deg=DEFAULT_PLANE_ANGLE_DEG,
):
    rng = random.Random(seed)
    club_name = normalize_club_name(club_name)
    model = make_plane_model(club_name, plane_angle_deg)
    launch_profile = get_plane_launch_profile(club_name)
    print_startup_diagnostics(model, club_name, plane_angle_deg)
    ...
    if best_valid_result is None:
        print("No valid 3D-plane PD downswing impact found yet.")
    print_result("Final best", final_result)
    print("Impact club vx:", round(final_result["impact_club_vx"], 4))
    print("Impact elbow bend deg:", round(math.degrees(final_result["impact_elbow_bend"]), 4))
...
def parse_args():
    parser = argparse.ArgumentParser(description="3D-plane CEM trainer using target poses plus PD control.")
