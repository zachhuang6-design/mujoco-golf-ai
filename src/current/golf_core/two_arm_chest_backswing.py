...
def two_stage_direction(start_vec, guide_vec, target_vec, progress):
    if progress < SHAFT_GUIDE_SPLIT:
        return rotate_direction_toward(start_vec, guide_vec, smootherstep(progress / SHAFT_GUIDE_SPLIT))
    return rotate_direction_toward(
        guide_vec,
        target_vec,
        smootherstep((progress - SHAFT_GUIDE_SPLIT) / (1.0 - SHAFT_GUIDE_SPLIT)),
    )
...
def two_stage_transport(vec, start_axis, guide_axis, target_axis, progress):
    ...
    if progress < SHAFT_GUIDE_SPLIT:
        return rotate_vec(vec, first_axis, first_angle * smootherstep(progress / SHAFT_GUIDE_SPLIT))
    ...
    return rotate_vec(
        carried_to_guide,
        second_axis,
        second_angle * smootherstep((progress - SHAFT_GUIDE_SPLIT) / (1.0 - SHAFT_GUIDE_SPLIT)),
    )


def legal_shaft_guide_direction(hand):
    side = 1.0 if hand == "right" else -1.0
    return normalize_vec((-0.35 * side, 0.55, 0.62))
...
def blend_points(start, target, top_progress, base, club_name, hand):
    ...
    residual_progress = smootherstep(top_progress)
...
def pose_points(hand=DEFAULT_HAND, progress=0.0, club_name=DEFAULT_CLUB):
    ...
    top_progress = smootherstep((progress - TAKEAWAY_PHASE) / (1.0 - TAKEAWAY_PHASE))
...
def print_backswing_report(hand, club_name=DEFAULT_CLUB):
    ...
    print("Top pause seconds:", TOP_PAUSE_SECONDS)
...
def main():
    ...
    top_pause_steps = max(1, int(TOP_PAUSE_SECONDS / sleep_time))
    takeaway_steps = max(1, int(BACKSWING_STEPS * TAKEAWAY_PHASE))
    top_motion_steps = max(1, BACKSWING_STEPS - takeaway_steps)
    step = 0
    total_steps = setup_hold_steps + takeaway_steps + takeaway_pause_steps + top_motion_steps + top_pause_steps
    with mujoco.viewer.launch_passive(model, data) as viewer:
        while viewer.is_running():
            if step < setup_hold_steps:
                ...
                progress = TAKEAWAY_PHASE
            elif step < setup_hold_steps + takeaway_steps + takeaway_pause_steps + top_motion_steps:
                motion_step = step - setup_hold_steps - takeaway_pause_steps
                progress = motion_step / BACKSWING_STEPS
            else:
                progress = 1.0
