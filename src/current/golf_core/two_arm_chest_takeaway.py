...
def lock_wrists_to_shaft(base, moved):
    base_grip = grip_center(base)
    base_shaft_axis = normalize_vec(sub_vec(base["clubhead"], base_grip))
    moved_grip = grip_center(moved)
    moved_shaft_axis = normalize_vec(sub_vec(moved["clubhead"], moved_grip))

    for side in ("left", "right"):
        wrist_key = f"{side}_wrist"
        base_offset = dot_vec(sub_vec(base[wrist_key], base_grip), base_shaft_axis)
        moved[wrist_key] = add_vec(moved_grip, scale_vec(moved_shaft_axis, base_offset))
...
def pose_points(hand=DEFAULT_HAND, progress=0.0):
    ...
    lock_wrists_to_shaft(base, moved)
